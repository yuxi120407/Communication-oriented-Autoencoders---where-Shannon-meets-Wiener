...
class ITLRegularizer(Layer):
    ...
    def default_distribution(shape):
        return K.random_normal(shape, std=10)

    def __init__(self, scale=1., distribution=default_distribution, distance='euclidean', ksize=1., pri_lambda=.1, **kwargs):
        super(ITLRegularizer, self).__init__(**kwargs)
        self.scale = scale
        self.distribution = distribution
        self.ksize = ksize
        self.distance = distance
        self.pri_lambda = pri_lambda
    ...
    def build(self):
        ...
        elif self.distance == 'pri': 
            #itl_divergence = - pri_lambda * K.log(K.mean(Gxz)**2 /
            #K.mean(Gzz)) - (1 - pri_lambda) * K.mean(Gxx)
            #itl_divergence = - self.pri_lambda * K.log(K.mean(Gxz) /
                             #K.sqrt(K.mean(Gxx)*K.mean(Gzz))) + K.mean(Gxx)
            #itl_divergence = self.pri_lambda * K.log(K.sqrt(K.mean(Gxx)*K.mean(Gzz)) /
                                   #K.mean(Gxz)) - K.mean(Gxx)
            itl_divergence = - self.pri_lambda * K.log(K.mean(Gxz)**2 /
                             (K.mean(Gzz)*K.mean(Gxx))) - K.log(K.mean(Gxx))
